...
fragpath <- "/d/tmp/regeps/regep00/fragments.tsv.gz"
test <- Read10X_h5("./RData/pbmc_granulocyte_sorted_10k_filtered_feature_bc_matrix.h5")
# get gene annotations for hg38
annotation <- GetGRangesFromEnsDb(ensdb = EnsDb.Hsapiens.v86)
seqlevels(annotation) <- paste0('chr', seqlevels(annotation))

# create a Seurat object containing the RNA adata
Rawlins_rna <- CreateSeuratObject(counts = counts, assay = "RNA")

# create ATAC assay and add it to the object
Rawlins_rna[["ATAC"]] <- CreateChromatinAssay(counts = counts$Peaks, sep = c(":", "-"), 
